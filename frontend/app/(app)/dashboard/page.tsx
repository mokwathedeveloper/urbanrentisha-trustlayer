...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${meta.tone}`}>{meta.label}</span>
  );
}
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
        {stats.map((stat) => (
          <div key={stat.label} className="ur-card p-4">
            <stat.icon className={`h-4 w-4 ${stat.color}`} />
            <p className="mt-2 text-xs text-ur-text-secondary">{stat.label}</p>
            <p className="text-xl font-black text-ur-navy">{loading ? "—" : stat.value}</p>
          </div>
        ))}
      </div>
