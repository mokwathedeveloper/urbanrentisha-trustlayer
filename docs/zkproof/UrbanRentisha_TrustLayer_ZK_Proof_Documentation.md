...
The deployed circuit now uses `MiMCPermutation(paymentSecret, paymentNonce)` — a minimal one-way compression function over the BLS12-381 scalar field using the `x^5` S-box (the same S-box exponent Poseidon uses for this field; `gcd(5, r-1) = 1` makes `x -> x^5` a bijection), with a fixed `paymentNonce` value injected into every round (Davies-Meyer style — not a true Feistel network, despite an earlier revision's name; there is no left/right swap, just a single accumulating state) and fixed, deterministically-generated round constants (`sha256` of a labelled seed, reduced mod the field order — see `circuits/payment-proof/payment_proof.circom` for the exact construction and `field.util.ts`'s `mimcPermutation` for the byte-identical off-chain twin used by the backend). This has no known efficient inversion, so finding an alternate (secret, nonce) pair that produces the same commitment is intractable.

**Round count — 220, not an arbitrary smaller number.** An `x^5` S-box gives an algebraic degree of `5^n` after `n` rounds; for polynomial-interpolation / Groebner-basis attacks to be infeasible, that degree must exceed the field size (`2^255` here). An earlier revision of this circuit shipped with 12 rounds — `5^12 ≈ 2^28`, roughly 18x short of the minimum `~110` rounds the math requires, let alone a safety margin. That gap was caught and fixed before this revision shipped: 220 rounds matches circomlib's own documented margin for an `x^5` MiMC construction over a similarly-sized field (`mimcsponge.circom`: "log_5(field) ~= 110 => nRounds should be 220").

**Documented MVP limitation:** this MiMCPermutation construction is **not** an independently audited Poseidon/MiMC parameter set. It is a minimal, self-contained construction sized to the same security margin circomlib documents for its own MiMC implementation. Production use should adopt a standard, audited primitive matched to this field (e.g. a published Poseidon instantiation for BLS12-381).
